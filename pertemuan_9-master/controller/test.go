...
func (c *ctrl) GetData(w http.ResponseWriter, r *http.Request) {
	...
	redisData, err := REDIS.Get(ctx, "list_data_mhs").Result()
	if err != nil {
...
func (c *ctrl) GetDataMhsDetail(w http.ResponseWriter, r *http.Request) {
	idmhs := chi.URLParam(r, "id")

	err, listmhsdata := c.us.GetDataDetailMhs(idmhs)
	if err != nil {
		log.Println("isi err ", err)
		ResponseApi(w, 500, nil, "Internal Server Error")
		return
	}
	ResponseApi(w, 200, listmhsdata, "Sukses Get  Data")
}
